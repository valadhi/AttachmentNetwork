...
class ReconstructerBatch(object):
  def __init__(self, input, theanoGenerator, weights, biases,
             visibleActivationFunction, hiddenActivationFunction,
             visibleDropout, hiddenDropout, fixedLabel, cdSteps):
...
def reconstructFixedLbl(biases, weights, dataInstances, 
  activationFun, label, steps):
  visibleReconstructions = np.concatenate((dataInstances, label), axis=1)
  for s in xrange(steps):
    hidden = updateLayer(Layer.HIDDEN, visibleReconstructions, biases, weights,
                       activationFun, True)
    visibleReconstructions = updateLayer(Layer.VISIBLE, hidden,
                      biases, weights, activationFun, False)
    visibleReconstructions[:,625:] = label
  '''
